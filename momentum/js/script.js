...
function showDay() {
  ...
  const d = `${new Intl.DateTimeFormat("ru", {
    weekday: "long",
  }).format(currentDay)},  ${currentDay.getDate()}, ${new Intl.DateTimeFormat("ru", {month: "long",}).format(currentDay)}`;
  date.textContent = d;
      }
...
console.log(city.value);
    ...
    let url;
      if (city.value == '') {
        if (hash == 'en' ) city.value = 'Minsk';
        if (hash == 'ru' ) city.value = 'Минск';
...
console.log(url);
      ...
      try {
       ...
       weatherError.textContent = '';
        ...
        wind.textContent = `${langArr['wind'][hash]}: ` + `${Math.round(data.wind.speed)} m/s`;
